...
for i in range(0,10):  
    ...
    # 创建sheet，并命名为“豆瓣Top250”，并在表头写入Number,Title和Info  
    ...
    for (idx, item) in enumerate(movie_titles):    
        ...
        #item.a.text.strip()的意思为：提取item中含有<a></a>标签中的内容
        #movie_info[idx].p.text.strip()的意思为
        #按照索引提取movie_info中含有<p></p>标签的内容
        #使用strip()去除首尾的格式（如回车、空格等）  
